...
app.post("/getMemberList", (req, res) => {
  connection.query("CALL getCampMember(?)", req.body.campId, function(
    err,
    rows,
    fields
  ) {
...
app.post("/getCampMemberTotal", (req, res) => {
  connection.query("CALL getCampMemberTotal(?)", req.body.campId, function(
    err,
    rows,
    fields
  ) {
...
app.post("/getAnnouncement", (req, res) => {
  connection.query("CALL getAnnouncement(?)", req.body.campId, function(
    err,
    rows,
    fields
  ) {
...
app.post("/addAnnouncement", (req, res) => {
  connection.query(
    ...
    [req.body.campId, req.body.aName, req.body.description, req.body.isPublic],
    function(err, result) {
      if (!err) {
        res.setHeader("Content-Type", "application/json");
        res.send(JSON.stringify(result));
...
app.post("/deleteAnnouncement", (req, res) => {
  connection.query(
    ...
    [req.body.campId, req.body.announcementID],
    function(err, result) {
      if (!err) {
        res.setHeader("Content-Type", "application/json");
        res.send(JSON.stringify(result));
